 ...
 * @test
 *
 * @summary converted from VM Testbase nsk/jdi/ArrayReference/setValues_ilii/setvaluesilii003.
 ...
 *     nsk/jdi/ArrayReference/setValues_ilii/setvaluesilii003 checks assertion:
 *     public void setValues(int index, List values, int srcIndex, int length)
 *     1. IndexOutOfBoundsException is thrown if the list is smaller then the
 *            array. Array has components of primitive types only.
 *     Debuggee defines eight sample array fields. One for each primitive type.
 *     Also, it defines tested array fields. For each primitive type a sample
 *     array has at least one element less then correspondent tested array.
 ...
 *     setValues(0, List, 0, List.size()) is invoked to set all elements of sample
 *     array from index 0 to ArrayReference from index 0. Since sample array has
 *     at least one element less tested array, IndexOutOfBoundsException is
 *     expected.
 * COMMENTS
 *     The tests shows the bug
 *     4448721: ArrayReference.setValues(int, List, int, int) doesn't throw
 *              IndexOutOfBoundsException
 * ===========================
 * To fix the bug 4491099,
 * the string
 *     arrayRef.setValues(0, listToSet, 0, listToSet.size());
 * is replaced with
