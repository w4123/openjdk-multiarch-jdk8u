...
OGLSD_SetScratchSurface(JNIEnv *env, jlong pConfigInfo)
{
    ...
    // switching to scratch context may result in black window issue
    // so will keep the current context
    if (OGLC_GET_VENDOR(oglc) == OGLC_VENDOR_ETNAVIV) {
        J2dRlsTraceLn(J2D_TRACE_INFO, "OGLSD_SetScratchContext: no-op");
        return OGLRenderQueue_GetCurrentContext();
    }
