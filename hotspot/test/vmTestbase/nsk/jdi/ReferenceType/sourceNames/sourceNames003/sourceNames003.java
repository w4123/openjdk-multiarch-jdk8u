 ...
 * @test
 *
 * @summary converted from VM Testbase nsk/jdi/ReferenceType/sourceNames/sourceNames003.
 * VM Testbase keywords: [quick, jpda, jdi, feature_sde, vm6]
 * VM Testbase readme:
 * DESCRIPTION
 *     The test checks up that method 'com.sun.jdi.ReferenceType.sourceNames(String stratum)' returns all available source names for all specified stratums.
 *     Debugger creates copy of class file for class 'nsk.share.jdi.TestClass1' with SourceDebugExtension attribute
 *     which contains informations for 3 stratums('TestStratum1'-'TestStratum3') and for each stratum 3 source names are defined.
 ...
 *     method 'com.sun.jdi.ReferenceType.sourceNames(String stratum)' returns all available source names for all stratums (for 'Java' stratum and for 'TestStratum1'-'TestStratum3').
 *
 * @library /vmTestbase
 *          /test/lib
...
public class sourceNames003 extends SDEDebugger {
    ...
    public static int run(String argv[], PrintStream out) {
        return new sourceNames003().runIt(argv, out);
    }
    ...
    public void doTest() {
        ...
        /*
         * Method 'prepareDefaultPathcedClassFile_Type2' creates class file with
        ...
        for (String stratumName : testStratumData.keySet()) {
            log.display("Check sources for stratum: '" + stratumName + "'");

            LocationsData locationsData = testStratumData.get(stratumName);

            // create list with expected sources
            List<String> sourceNames = new ArrayList<String>();
            sourceNames.addAll(locationsData.sourceLocations.keySet());

            check_ReferenceType_sourceNames(referenceType, stratumName, sourceNames);
        }
