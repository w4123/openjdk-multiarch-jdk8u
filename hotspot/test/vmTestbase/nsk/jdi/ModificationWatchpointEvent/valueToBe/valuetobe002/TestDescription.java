 ...
 * @test
 *
 * @summary converted from VM Testbase nsk/jdi/ModificationWatchpointEvent/valueToBe/valuetobe002.
 ...
 *      which is equal to previous value, then Value reference returned
 *      by ModificationWatchpointEvent.valueToBe() is equal to one
 *      returned by Watchpoint.valueCurrent().
 ...
 *    from debuggee.
 *    Upon receiving <GO> command from debugger, the debuggee assignes every field
 *    to itself (i.e. the old and new values of the field are equal) and sends
 *    command <DONE>.
 ...
 *    these values are not equal. The test also fails if no
 *    ModificationWatchpointEvent was received.
 * COMMENTS
