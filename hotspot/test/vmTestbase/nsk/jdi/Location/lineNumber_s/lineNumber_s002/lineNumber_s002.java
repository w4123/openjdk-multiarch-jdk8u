 ...
 * @test
 *
 * @summary converted from VM Testbase nsk/jdi/Location/lineNumber_s/lineNumber_s002.
 * VM Testbase keywords: [quick, jpda, jdi, feature_sde, vm6]
 * VM Testbase readme:
 * DESCRIPTION
 *     The test checks up that method 'com.sun.jdi.Location.lineNumber(String stratum)' returns -1 if line number
 *     is unavailable for given stratum.
 ...
 *     and checks up that all locations obtained for one the 'TestStratumXXX' returns -1 as line number for all others 'TestStratumXXX'.
 *
 * @library /vmTestbase
 *          /test/lib
...
public class lineNumber_s002 extends SDEDebugger {
    ...
    public static int run(String argv[], PrintStream out) {
        return new lineNumber_s002().runIt(argv, out);
    }
    ...
    public void doTest() {
        ...
        for (String stratumName1 : testStratumData.keySet()) {
            ...
            for (Location location : allLocations) {
                ...
                // Location.lineNumber() returns -1
                for (String stratumName2 : testStratumData.keySet()) {
                    if (!stratumName2.equals(stratumName1)) {
                        log.display("Try get line number for stratum: " + stratumName2);

                        int lineNumber = location.lineNumber(stratumName2);

                        if (lineNumber != -1) {
                            setSuccess(false);
                            log.complain("Location.lineNumber returns unexpected value: " + lineNumber
                                    + ", expected is " + -1);
                        }
                    }
                }
