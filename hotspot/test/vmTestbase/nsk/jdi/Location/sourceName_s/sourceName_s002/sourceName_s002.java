 ...
 * @test
 *
 * @summary converted from VM Testbase nsk/jdi/Location/sourceName_s/sourceName_s002.
 * VM Testbase keywords: [quick, jpda, jdi, feature_sde, vm6]
 * VM Testbase readme:
 * DESCRIPTION
 *     The test checks up that method 'com.sun.jdi.Location.sourceName(String stratum)' throws AbsentInformationException
 *     if source name is unavailable for given stratum.
 ...
 *     and checks up that for all locations obtained for one the 'TestStratumXXX' method 'sourceName(String stratum)' throws AbsentInformationException
 *     when try to get source name for all others 'TestStratumXXX'.
 *
 * @library /vmTestbase
 *          /test/lib
...
public class sourceName_s002 extends SDEDebugger {
    ...
    public static int run(String argv[], PrintStream out) {
        return new sourceName_s002().runIt(argv, out);
    }
    ...
    public void doTest() {
        ...
        for (String stratumName1 : testStratumData.keySet()) {
            ...
            for (Location location : allLocations) {
                ...
                // Location.sourceName() throws AbsentInformationException
                for (String stratumName2 : testStratumData.keySet()) {
                    if (!stratumName2.equals(stratumName1)) {
                        log.display("Try get source name for stratum: " + stratumName2);

                        try {
                            location.sourceName(stratumName2);
                            setSuccess(false);
