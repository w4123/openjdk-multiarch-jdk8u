...
void JNICALL FieldAccess(jvmtiEnv *jvmti_env, JNIEnv *env,
    ...
    if (printdump == JNI_TRUE) {
        printf(">>> FieldAccess event, fieldID: 0x%p\n", field);
    }
    actual_fid = field;
}

#ifdef STATIC_BUILD
JNIEXPORT jint JNICALL Agent_OnLoad_setfldw003(JavaVM *jvm, char *options, void *reserved) {
    return Agent_Initialize(jvm, options, reserved);
}
JNIEXPORT jint JNICALL Agent_OnAttach_setfldw003(JavaVM *jvm, char *options, void *reserved) {
    return Agent_Initialize(jvm, options, reserved);
}
JNIEXPORT jint JNI_OnLoad_setfldw003(JavaVM *jvm, char *options, void *reserved) {
...
Java_nsk_jvmti_SetFieldAccessWatch_setfldw003_getReady(JNIEnv *env, jclass cls) {
    jvmtiError err;
    size_t i;
...
JNIEXPORT void JNICALL
Java_nsk_jvmti_SetFieldAccessWatch_setfldw003_check(JNIEnv *env,
        jclass cls, jint ind) {
    if (printdump == JNI_TRUE) {
    ...
    if (actual_fid != fids[ind]) {
        result = STATUS_FAILED;
        printf("Field fld%d: thrown field ID expected=0x%p, actual=0x%p\n",
...
Java_nsk_jvmti_SetFieldAccessWatch_setfldw003_getRes(JNIEnv *env, jclass cls) {
    return result;
}
