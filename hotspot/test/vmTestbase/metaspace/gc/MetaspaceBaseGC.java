...
public abstract class MetaspaceBaseGC {
    ...
    protected void configure(String args[]) {
        ...
        for (String arg: vmArgs) {
            System.out.println("configure: arg" + arg);
            if (arg.startsWith("-Xloggc:") && arg.length() > 8) {
               gclogFileName = arg.substring(arg.lastIndexOf(':') + 1);
               System.out.println("configure: gclogFileName = " + gclogFileName);
            }
        }
        parseArgs(args);
    }
     ...
     * It's supposed that the test is executed with -Xloggc:gc.log option.
     *
