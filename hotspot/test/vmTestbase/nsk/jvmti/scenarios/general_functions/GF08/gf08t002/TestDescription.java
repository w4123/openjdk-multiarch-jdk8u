 ...
 * @test
 *
 * @summary converted from VM Testbase nsk/jvmti/scenarios/general_functions/GF08/gf08t002.
 ...
 *    Functions:
 *        Do the following:
 *        Run simple java apllication with VM option '-verbose:class'.
 *        Run the same application with JVMTI agent. The agent should
 *        set JVMTI_VERBOSE_CLASS with SetVerboseFlag. Check that outputs
 *        in stderr in both runs are equal.
 *    The test agent has a special input parameter 'setVerboseMode'.
 *    When VM runs the test class 'gf08t002' with
 *      '-agentlib:gf08t002=setVerboseMode=yes'
 *    option, then the agent calls SetVerboseFlag with
 *    JVMTI_VERBOSE_CLASS flag in Onload phase.
 *    The test's script wrapper runs the 'gf08t002' class twice.
 *    First time, with "setVerboseMode=yes" agent mode. Second
 *    time, with "setVerboseMode=no" agent mode and with
 *    "-verbose:class" VM option. In both cases the output is
 *    searched for 'Loaded ' string. The test fails if this
