 ...
 * @test
 *
 * @summary converted from VM Testbase nsk/jdi/StackFrame/setValue/setvalue002.
 ...
 *     The debugger program - nsk.jdi.StackFrame.setValue.setvalue.002;
 *     the debuggee program - nsk.jdi.StackFrame.setValue.setvalue.002a.
 ...
 * @build nsk.jdi.StackFrame.setValue.setvalue002.setvalue002
 *        nsk.jdi.StackFrame.setValue.setvalue002.setvalue002a
 *
 * @comment make sure setvalue002a is compiled with full debug info
 ...
 * <BR>
 * The test checks up throwing                                  <BR>
 *    IllegalArgumentException  and                             <BR>
 ...
 *    Debuggee's method runt1() calls to the method runt2().            <BR>
 *    At the beginning the debugger sets up breakpoints at both         <BR>
 *    methods, so that, in run-time runt2() is suspended first.         <BR>
 *    After getting the thread2 suspended first time, the debugger      <BR>
 *    gets a LocalVariable object locvar1, mirroring                    <BR>
 *    a boolean variable in the runt2(), which becomes invalid          <BR>
 *    when runt2() is left and the thread2 is suspended within run1t(). <BR>
 *    After getting the thread2 suspended second time,                  <BR>
 *    the debugger checks up that the method invocation                 <BR>
 *    StackFrame.setValue() on locvar1 does throw                       <BR>
 *    IllegalArgumentException.                                         <BR>
 *    Then the debugger gets a LocalVariable locvar2 mirroring          <BR>
 *    a byte variable in current (valid) StackFrame and                 <BR>
 *    checks up that the method invocation                              <BR>
 *        setValue(locvar2, byte argument)                              <BR>
 *    does throw InvalidTypeException.                                  <BR>
 * <BR>
 */
...
public class setvalue002 {
    ...
    private String testedClassName =
        "nsk.jdi.StackFrame.setValue.setvalue002.Threadsetvalue002a";
    ...
    private int runThis (String argv[], PrintStream out) {
        ...
        log2("setvalue002a debuggee launched");
        debuggee.resume();
    ...
    //------------------------------------------------------  testing section
        ...
        pipe.println("newcheck");
            ...
            label1: {
                ...
                String bl4 = "bl4";
                String bl1 = "bl1";
                String locThread = "locThread";

                log2("      getting LocalVariables for checking up ");
                try {
                    locvar1 = stackFrame.visibleVariableByName(bl3);
                    locvar2 = stackFrame.visibleVariableByName(bl4);
                } catch ( Exception e ) {
                ...
                log2("      getting thread2's current StackFrame");
                try {
                    stackFrame = thread2.frame(0);
                } catch ( Exception e ) {
                    log3("ERROR: Exception for stackFrame = thread2.frame(0)    :" + e);
                ...
                try {
                    locvar2 = stackFrame.visibleVariableByName(bl1);
                    blv2 = (BooleanValue) stackFrame.getValue(locvar2);

                    log2("      checking up IllegalArgumentException");
                    stackFrame.setValue(locvar1, blv2);

                    log3("ERROR:  no IllegalArgumentException after thread2 returned from runt2()");
                    expresult = returnCode1;
                } catch ( IllegalArgumentException e1 ) {
                    log2("     :  IllegalArgumentException after thread2 returned from runt2()");
                } catch ( InvalidTypeException e2 ) {
                    log3("ERROR:  InvalidTypeException for stackFrame.setValue(locvar1, blv2)");
                    expresult = returnCode1;
                } catch ( Exception e ) {
                    log3("ERROR:  unexpected exception:  " + e);
                    expresult = returnCode1;
                ...
                try {
                    locvar2 = stackFrame.visibleVariableByName(locThread);

                    log2("      checking up InvalidTypeException");
                    stackFrame.setValue(locvar2, blv2);

                    log3("ERROR:  no InvalidTypeException");
                    expresult = returnCode1;
                } catch ( InvalidTypeException eInavalid ) {
