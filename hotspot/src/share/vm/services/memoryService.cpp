...
void MemoryService::add_g1YoungGen_memory_pool(G1CollectedHeap* g1h,
  ...
  G1EdenPool* eden = new G1EdenPool(g1h, g1h->g1mm()->eden_space_committed());
  G1SurvivorPool* survivor = new G1SurvivorPool(g1h, g1h->g1mm()->survivor_space_committed());
...
void MemoryService::add_g1OldGen_memory_pool(G1CollectedHeap* g1h,
  ...
  G1OldGenPool* old_gen = new G1OldGenPool(g1h,g1h->g1mm()->old_space_committed(), g1h->max_capacity());
